			...
			for (var i in hound.promociones) {						
				...
				$.ajax({
					...
					error : function(xhr,err) {
						alert("Error al asignar Imagen a la promocion");					    					
					}
				});					
				$.ajax({
					type : "GET",	
					...
					error : function(xhr,err) {						
						alert("Error al asignar Icono a la promocion");							
					}
				});					
...
hound.getTema = function() {
	...
	.ajax({
		...
		error : function(err, data) {
			alert("Error get Tema" + JSON.stringify(err) + "::"
					+ data)
		}
	});
};
